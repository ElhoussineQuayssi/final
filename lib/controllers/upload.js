...
export class UploadController {
  static async uploadImage(user, file, uploadData) {
    ...
    if (!type || !['project-main', 'project-gallery', 'blog'].includes(type)) {
      throw new Error('Invalid upload type. Must be project-main, project-gallery, or blog.');
    }
    ...
    const { data: uploadedData, error: uploadError } = await supabaseServer.storage
      .from(bucketName)
