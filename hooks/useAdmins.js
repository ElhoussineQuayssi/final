...
export function useAdmins() {
  ...
  const updateAdmin = async (adminId, updatedData) => {
    const { error: updateError } = await supabaseClient.from('admins').update(updatedData).eq('id', adminId);
    if (updateError) {
  ...
  const deleteAdmin = async (adminId) => {
    const { error: deleteError } = await supabaseClient.from('admins').delete().eq('id', adminId);
    if (deleteError) {
