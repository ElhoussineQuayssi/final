...
const ProjectGallery = ({ images, projectTitle, className = "" }) => {
  ...
  const processedImages = useMemo(() => {
    ...
    return images
      .map(imageSrc => {
        ...
        return cleanUrl && (cleanUrl.startsWith('http') || cleanUrl.startsWith('/')) ? cleanUrl : null;
      })
      .filter(Boolean);
  }, [images]);
  ...
  return (
    ...
    <div className={`grid grid-cols-2 md:grid-cols-3 gap-4 transition-opacity duration-500 ${hasIntersected ? 'opacity-100' : 'opacity-0'}`}>
