...
const ImageText = ["https://hpymvpexiunftdgeobiw.supabase.co/storage/v1/object/public/projects/Centre/IDC08872.JPG",
...
export default function Home() {
  ...
  const { projects: allProjects, isLoading: projectsLoading, isError: projectsError } = useProjects();
  const { blogs: allBlogs, isLoading: blogsLoading, isError: blogsError } = useBlogs();
  ...
  const projects = allProjects?.slice(0, 3).map(project => ({
    ...
    link: project.slug ? `/projects/${project.slug}` : `/projects/${project.id}`,
  })) || [];
       ...
       <div className="relative">
         ...
         <div className="relative py-20 px-8 rounded-3xl border border-white/50 shadow-inner">
           ...
           <div className="mt-16">
             <ContentGrid
               items={[
                 { value: "6+", title: "Projets Actifs", description: "Initiatives actives à travers le Maroc" },
                 { value: "1000+", title: "Bénéficiaires Aidés", description: "Vies transformées par nos actions" },
                 { value: "98%", title: "Taux de Satisfaction", description: "Satisfaction de nos bénéficiaires" },
               ]}
               ...
               renderItem={(stat, index) => (
                 ...
                 >
                   <StatsCard value={stat.value} title={stat.title} description={stat.description} />
                 </div>
               )}
             />
           </div>
