...
const BackgroundSlideshow = ({ images, currentIndex }) => {
    ...
    console.log('BackgroundSlideshow images:', images, 'currentIndex:', currentIndex);

    return (
        <div className="absolute inset-0 overflow-hidden">
            {images.map((image, index) => {
                // Clean image URL if it has quotes
                const cleanImage = typeof image === 'string' ? image.replace(/^"|"$/g, '') : image;
...
const UnifiedHero = ({ title, subtitle, images = [] }) => {
  ...
  const { projects, isLoading } = useProjects();
    ...
    // Generate random images from projects if none provided
