...
export function useProjectImages(projectId = null) {
  ...
  const createProjectImage = async (projectImageData) => {
    const { error: createError } = await supabaseClient.from('project_images').insert([projectImageData]);
    if (createError) {
  ...
  const updateProjectImage = async (projectImageId, updatedData) => {
    const { error: updateError } = await supabaseClient.from('project_images').update(updatedData).eq('id', projectImageId);
    if (updateError) {
  ...
  const deleteProjectImage = async (projectImageId) => {
    const { error: deleteError } = await supabaseClient.from('project_images').delete().eq('id', projectImageId);
    if (deleteError) {
